...
#ifdef REV2B
	#define en_gate_pha()  do{ HI(GPIOD, 8); }while(0)
	#define dis_gate_pha() do{ LO(GPIOD, 8); }while(0)
	#define en_gate_phb()  do{ HI(GPIOH, 9); }while(0)
	#define dis_gate_phb() do{ LO(GPIOH, 9); }while(0)
#endif
...
void charger_safety_shutdown()
{
	dis_gate_pha();
	dis_gate_phb();
	// Make sure the gate drive enable outputs that we just set are actually driven low:
	#ifdef REV2A
		IO_TO_GPO(GPIOG, 1);
		IO_TO_GPO(GPIOG, 0);
	#endif
	...
	run_pump = 0;
	infet_chargepump_0();
	IO_TO_GPO(GPIOE, 10); // same pin for REV2A, REV2B
}
...
// Test macros to manually drive the IOs normally driven by HRTIM. Not used anywhere.
#if 0
...
void init_charger()
{
	...
	infet_chargepump_0();
	IO_TO_GPO(GPIOE, 10); // same for REV2A,B

	pulseout_0();
	IO_TO_GPO(GPIOE, 11); // same for REV2A,B
...
//	uart_print_string_blocking("#");
	...
	set_current(1500); // Sets the DAC output register value properly
...
*/
	...
	// period and compare registers must be between 0x3 and 0xFFFD.
