...
char* o_utoa8_hex(uint8_t val, char* str)
{
	...
	if(first >= 10) *str++ = first-10+'a'; else *str++ = first+'0';
	if(second >= 10) *str++ = second-10+'a'; else *str++ = second+'0';
	*str = 0;
	return str;
}
...
{
	str = o_utoa8_hex((val&0xff00)>>8, str);
	str = o_utoa8_hex((val&0x00ff)>>0, str);
	return str;
}

char* o_utoa32_hex(uint32_t val, char* str)
{
	str = o_utoa8_hex((val&0xff000000)>>24, str);
	str = o_utoa8_hex((val&0x00ff0000)>>16, str);
	str = o_utoa8_hex((val&0x0000ff00)>>8, str);
	str = o_utoa8_hex((val&0x000000ff)>>0, str);
	return str;
}
